...
export default function CourtroomClient({
  ...
  async function sendArgument() {
    ...
    if (input.length > 1000) {
      setFeedback("Argument too long. Keep it under 1000 characters.");
      return;
    }
    setAiError(false);
    if (mode === "defense") {
  ...
  async function sendDefenseArgument() {
    ...
    try {
      ...
      setScoreHistory(data.updatedHistory);
    } catch (err) {
      console.error(err);
      setAiError(true);
  ...
  async function sendWitnessQuestion() {
    ...
    try {
      ...
      console.error(err);
      setAiError(true);
      setWitnessMessages((prev) => prev.slice(0, -1));
    } finally {
      setLoading(false);
        ...
        >
          ...
          <div>
            ...
            <span style={{ fontSize: 12, color: "#6B7280" }}>
              {caseData.witness.role} · On the stand
            </span>
          </div>
          {contradiction && (
            <div
