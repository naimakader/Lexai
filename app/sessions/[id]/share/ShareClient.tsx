...
export default function ShareClient({ session }: { session: any }) {
  ...
  const scoreColor = scoreNum >= 70 ? "#4ADE80" : scoreNum >= 40 ? "#FBBF24" : "#F87171"
  const scoreBg = scoreNum >= 70 ? "rgba(74,222,128,0.1)" : scoreNum >= 40 ? "rgba(251,191,36,0.1)" : "rgba(248,113,113,0.1)"
  const scoreBorder = scoreNum >= 70 ? "rgba(74,222,128,0.25)" : scoreNum >= 40 ? "rgba(251,191,36,0.25)" : "rgba(248,113,113,0.25)"
  const grade = scoreNum >= 80 ? "A" : scoreNum >= 65 ? "B" : scoreNum >= 50 ? "C" : "D"
  const gradeLabel = scoreNum >= 80 ? "Excellent" : scoreNum >= 65 ? "Good" : scoreNum >= 50 ? "Average" : "Needs Work"
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#03030A", color: "#F0EEE8" }}>
      ...
      <nav style={{
        ...
        <Link href={`/sessions/${session.id}`} style={{ fontSize: 13, color: "#6B7280", textDecoration: "none" }}>
          ← Back to session
        </Link>
      </nav>

      <div style={{ position: "relative", zIndex: 10, maxWidth: 760, margin: "0 auto", padding: "3rem 2rem" }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{
                width: 28, height: 28, borderRadius: 7, background: "#6366F1",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontWeight: 800, fontSize: 12, color: "#fff",
              }}>L</div>
              <span style={{ fontSize: 15, fontWeight: 800, color: "#fff", letterSpacing: "-0.02em" }}>LexAI</span>
            </div>
            <span style={{
          ...
          <div style={{ padding: "2rem" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: "1.5rem" }}>
              <div style={{
                ...
                <span style={{ display: "block", fontSize: "2.2rem", fontWeight: 800, color: scoreColor, lineHeight: 1, marginBottom: 4 }}>
                  {scoreNum}
                </span>
                <span style={{ fontSize: 11, color: scoreColor, opacity: 0.8, fontFamily: "monospace", fontWeight: 600, letterSpacing: "0.06em" }}>
                  FINAL SCORE
                </span>
              </div>
              <div style={{
                ...
                <span style={{ display: "block", fontSize: "2.2rem", fontWeight: 800, color: "#fff", lineHeight: 1, marginBottom: 4 }}>
                  {turns}
                </span>
                <span style={{ fontSize: 11, color: "#6B7280", fontFamily: "monospace", fontWeight: 600, letterSpacing: "0.06em" }}>
                  ARGUMENTS
                </span>
              </div>
              <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                "{bestArgument.length > 160 ? bestArgument.slice(0, 160) + "..." : bestArgument}"
              </p>
            </div>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 12, color: "#4B5563" }}>lexai.vercel.app</span>
            <span style={{ fontSize: 12, color: "#374151" }}>Practice law. Win cases. Free.</span>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {/* LinkedIn share */}
            ...
            style={{
              ...
              borderRadius: 14, cursor: "pointer",
              fontSize: 15, fontWeight: 700, color: "#60A5FA",
          ...
          {/* Twitter share */}
            ...
            href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`I just argued ${session.case_title} on LexAI and scored ${scoreNum}/100. Free AI courtroom simulator for law students.`)}&url=${encodeURIComponent(shareUrl)}`}
            target="_blank"
            rel="noopener noreferrer"
            style={{
              width: "100%", padding: "16px",
